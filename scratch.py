# -*- coding: utf-8 -*-
"""
Scratch code for the Tieto Recommendation engine.
...
class user_state(object):
    ...
    def __init__(self, imei, activity, location, timestamp, song):
        ...
        # TODO : Finish the checking of the arguments for the test code
        ...
        if np.floor(np.log10(imei)) != 14:
            raise Exception('Given IMEI has not the proper length (15 digits).')
...
class song(object):
    ...
    def __init__(self, query_string):
        ...
        self.release_id = self._look_up_release_id()
        self.genre, self.style, self.tempo, self.year, self.country = \
            self._look_up_details_by_master_id()
        self.sens_me = self._sens_me()


    def _look_up_release_id(self):
        """
        ...
        return release.data['master_id']


    def _look_up_details_by_master_id(self):
        """
...
def distance_user_states(userstate1, userstate2):
    ...
    """
    if not isinstance(userstate1, user_state):
        raise Exception('Given first user state is not a valid one.')
    if not isinstance(userstate2, user_state):
    ...
    distance = {'location': distance_locations(userstate1.location, \
                ...
                'activity': distance_activities(userstate1.activity, \
                                                userstate2.activity),
                'song': distance_songs(userstate1.song, userstate2.song),
...
def distance_songs(song1, song2):
    ...
    distance_set = {'distance_genre': distance_genres(song1.genre, \
                    ...
                    'distance_tempo': distance_tempos(song1.tempo, \
                                                        song2.tempo),
                    'distance_sens_me': distance_sens_me(song1.sens_me, \
                                                        song2.sens_me),
                    }
    return distance_set
